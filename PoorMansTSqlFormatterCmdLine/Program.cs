...
// sort out language
var _generalResourceManager = getResourceManager(uiLangCode, ref showUsageError);
...
{
    showUsageError = true;
    writeTranslatedErrorLine("NoInputErrorMessage");
}
else if ((!string.IsNullOrEmpty(stdInput) && remainingArgs.Count == 1) || remainingArgs.Count > 1)
{
    showUsageError = true;
    writeTranslatedErrorLine("UnrecognizedArgumentsErrorMessage");
}
...
if (showUsageFriendly || showUsageError)
{
    ...
    outStream.WriteLine(getResourceString("ProgramSummary"));
    outStream.WriteLine("v" + Assembly.GetExecutingAssembly().GetName().Version?.ToString());
    outStream.WriteLine(getResourceString("ProgramUsageNotes"));
    return 1;
}

var formatter = new PoorMansTSqlFormatterLib.Formatters.TSqlStandardFormatter(options);
formatter.ErrorOutputPrefix = getResourceString("ParseErrorWarningPrefix") + Environment.NewLine;
var formattingManager = new PoorMansTSqlFormatterLib.SqlFormattingManager(formatter);

bool warningEncountered = false;
int? explicitErrorCode = null;

if (!string.IsNullOrEmpty(stdInput))
{
    FormatInputContent(allowParsingErrors, outputFileOrFolder, stdInput, formattingManager);
}
else
{
    FormatPathContent(allowParsingErrors, extensions, ref backups, recursiveSearch, outputFileOrFolder, remainingArgs, formattingManager);
}

if (explicitErrorCode != null)
    return (int)explicitErrorCode;
...
{
    return _generalResourceManager?.GetString(key) ?? String.Empty;
}

void writeTranslatedErrorLine(string key, params object[] args)
{
    Console.Error.WriteLine(String.Format(getResourceString(key), args));
}

ResourceManager getResourceManager(string? uiLangCode, ref bool showUsageError)
{
    ...
    if (uiLangCode != null)
    {
        string uiLangCode_upper = uiLangCode.ToUpperInvariant();
        if (!uiLangCode_upper.Equals(UILANGUAGE_EN)
            && !uiLangCode_upper.Equals(UILANGUAGE_FR)
            && !uiLangCode_upper.Equals(UILANGUAGE_ES)
            )
        {
            showUsageError = true;
            //get the resource manager with default language, before displaying error.
            var _generalResourceManager = new ResourceManager("PoorMansTSqlFormatterCmdLine.GeneralLanguageContent", Assembly.GetExecutingAssembly());
            Console.Error.WriteLine(_generalResourceManager.GetString("UnrecognizedLanguageErrorMessage"));
            return _generalResourceManager;
        }
        else
        {
            System.Threading.Thread.CurrentThread.CurrentUICulture = new System.Globalization.CultureInfo(uiLangCode);
            return new ResourceManager("PoorMansTSqlFormatterCmdLine.GeneralLanguageContent", Assembly.GetExecutingAssembly());
...
bool ProcessSearchResults(List<string> extensions, bool backups, bool allowParsingErrors, PoorMansTSqlFormatterLib.SqlFormattingManager formattingManager, FileSystemInfo[] matchingObjects, StreamWriter? singleFileWriter, string? replaceFromFolderPath, string? replaceToFolderPath, ref bool warningEncountered)
{
...
void ReFormatFile(FileInfo fileInfo, PoorMansTSqlFormatterLib.SqlFormattingManager formattingManager, bool backups, bool allowParsingErrors, StreamWriter? singleFileWriter, string? replaceFromFolderPath, string? replaceToFolderPath, ref bool warningEncountered)
{
    ...
    {
        writeTranslatedErrorLine("FileReadFailureWarningMessage", fileInfo.FullName);
        writeTranslatedErrorLine("ErrorDetailMessageFragment", ex.Message);
    ...
    if (oldFileContents.Length > 0)
    {
        ...
        if (parsingError)
        {
            writeTranslatedErrorLine("ParsingErrorWarningMessage", fileInfo.FullName);
            if (parseException != null)
                writeTranslatedErrorLine("ErrorDetailMessageFragment", parseException.Message);
            warningEncountered = true;
        }
    }
    if (!parsingError
...

    {
        if (backups)
        {
            ...
            {
                writeTranslatedErrorLine("BackupFailureWarningMessage", fileInfo.FullName, Environment.NewLine);
                writeTranslatedErrorLine("ErrorDetailMessageFragment", ex.Message);
                failedBackup = true;
                warningEncountered = true;
            }
        ...
        if (!failedBackup)
        {
            ...
            else
            {
                ...
                if (replaceFromFolderPath != null && replaceToFolderPath != null)
                {
                    ...
                    string targetFolder = Path.GetDirectoryName(fullTargetPath) ?? String.Empty;
                    try
                    {
                    ...
                    {
                        writeTranslatedErrorLine("FolderCreationFailureWarningMessage", targetFolder);
                        writeTranslatedErrorLine("ErrorDetailMessageFragment", ex.Message);
                        failedFolder = true;
                        warningEncountered = true;
                    }
...
void WriteResultFile(string targetFilePath, string? replaceFromFolderPath, string? replaceToFolderPath, ref bool warningEncountered, string? newFileContents)
{
    try
    {
    ...
    {
        writeTranslatedErrorLine("ContentWriteFailureWarningMessage", targetFilePath);
        writeTranslatedErrorLine("ErrorDetailMessageFragment", ex.Message);
        if (replaceFromFolderPath == null || replaceToFolderPath == null)
            writeTranslatedErrorLine("PossiblePartialWriteWarningMessage");
        warningEncountered = true;
    }
}

System.IO.FileSystemInfo[] WildcardFileSearch(bool recursiveSearch, out DirectoryInfo baseDirectory, string searchPattern, string baseDirectoryName)
{
    baseDirectory = new System.IO.DirectoryInfo(baseDirectoryName);
    if (searchPattern.Length > 0)
    {
        if (recursiveSearch)
            return baseDirectory.GetFileSystemInfos(searchPattern);
        else
            return baseDirectory.GetFiles(searchPattern);
    }
    else
    {
        if (recursiveSearch)
            return baseDirectory.GetFileSystemInfos();
        else
            return new FileSystemInfo[0];
    }
}

void FormatInputContent(bool allowParsingErrors, string? outputFileOrFolder, string? stdInput, PoorMansTSqlFormatterLib.SqlFormattingManager formattingManager)
{
    ...
    try
    {
        formattedOutput = formattingManager.Format(stdInput ?? String.Empty, ref parsingError);
    ...
    if (parsingError)
    {
        writeTranslatedErrorLine("ParsingErrorWarningMessage", "STDIN");
        if (parseException != null)
            writeTranslatedErrorLine("ErrorDetailMessageFragment", parseException.Message);
        warningEncountered = true;
    }
...
int? FormatPathContent(bool allowParsingErrors, List<string> extensions, ref bool backups, bool recursiveSearch, string? outputFileOrFolder, List<string> remainingArgs, PoorMansTSqlFormatterLib.SqlFormattingManager formattingManager)
{
    string searchPattern = Path.GetFileName(remainingArgs[0]);
    string baseDirectoryName = Path.GetDirectoryName(remainingArgs[0]) ?? string.Empty;
    ...
    System.IO.FileSystemInfo[] matchingObjects;
    try
    {
        matchingObjects = WildcardFileSearch(recursiveSearch, out baseDirectory, searchPattern, baseDirectoryName);
    }
    catch (Exception e)
    {
        writeTranslatedErrorLine("PathPatternErrorMessage", e.Message);
        return 2;
    }
    ...
    if (!string.IsNullOrEmpty(outputFileOrFolder))
    {
        ...
        else
        {
            ...
            {
                writeTranslatedErrorLine("OutputFileCreationErrorMessage", e.Message);
                return 3;
            }
        }
    }
    ...
    {
        writeTranslatedErrorLine("NoFilesFoundWarningMessage", remainingArgs[0], string.Join(",", extensions.ToArray()));
        return 4;
    }
